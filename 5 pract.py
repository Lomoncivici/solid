...
def create_estate():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            tx_hash = contract.functions.createEstate(address).transact({
                "from": account
            })
...
def create_ad():
    if request.method == 'POST':
        ...
        price = int(request.form['price']) * 10**18  # Переводим цену в wei
...
def update_ad():
    if request.method == 'POST':
        ...
        is_active = 'is_open' in request.form
        ...
        try:
            ...
            print(f"estate_id: {ad_id}, is_active: {is_active}")

            tx_hash = contract.functions.updateAdType(ad_id, is_active).transact({
                "from": account
            })
        except Exception as e:
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if account_balance < price:
                flash("Ошибка: Сервис временно не доступен.", "error")
                return redirect(url_for('buy_estate'))
...
def withdraw():
    if request.method == 'POST':
        ...
        amount = int(request.form['amount'])  # Получаем сумму из формы     
